...
class SituationProcessor {
  ...
  static Situation placeBakiOnField(BakiGame game, int fieldId) {
    final situationFields = game.situation.fields;
    final situationField = game.getSituationFieldById(fieldId);

    // generate cool new baki to add
    final size = game.level.fieldSize / 2.5;

    // bakiId can be set to the turn, then it will always be unique
    final bakiId = game.situation.turns.toString();
    final bakiData = BakiGenerator()
        .generate(size, color: game.situation.turnPlayer.color, id: bakiId);
    final baki = BakiLayout(bakiData);

    situationField.bakis
        .add(Baki(fromPlayer: game.situation.turnPlayer, bakiLayout: baki));
